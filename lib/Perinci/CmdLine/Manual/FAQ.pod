...
This document describes version 0.54 of Perinci::CmdLine::Manual::FAQ (from Perl distribution Perinci-CmdLine-Lite), released on 2014-11-23.
...
Currently bash and tcsh shells are supported. Basically, you activate completion
by invoking C<complete -C ProgramName ProgramName> in bash (or C<complete
ProgramName 'p/*/`ProgramName`/'> in tcsh). This means, to get completion bash
will execute the program with COMP_LINE and COMP_POINT environment set to the
command-line and cursor position, respectively (in tcsh, COMMAND_LINE is given
instead). P::C will detect these environment variables and do the completion for
you (either providing list of subcommands, list of available options, or
argument values). The process is quite transparent to you and is convenient.

Another alternative used by many is to generate C<complete> commands statically.
